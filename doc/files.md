...
The data directory is the default location where the Novacoin Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.novacoin/`
macOS    | `$HOME/Library/Application Support/Novacoin/`
Windows  | `%LOCALAPPDATA%\Novacoin\` <sup>[\[1\]](#note1)</sup>
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Novacoin blocks (dumped in network format, 128 MiB per file)
...
`./`               | `novacoin.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `novacoind` or `novacoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [novacoin.conf](bitcoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`novacoin-qt` uses [`QSettings`](https://doc.qt.io/qt-6/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-6/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Novacoin Core:
...
This table describes the files installed by Novacoin Core across different platforms.

| **Path**                                                   | **Description**                                                             |
|------------------------------------------------------------|-----------------------------------------------------------------------------|
| [README.md](/README.md) or [readme.txt](README_windows.txt) | Project information and instructions                                        |
| novacoin.conf                                              | [Generated](../contrib/devtools/gen-bitcoin-conf.sh) configuration file     |
| bin/novacoin                                               | Command-line tool for interacting with Novacoin. Calls other binaries below. |
| bin/novacoin-cli                                           | Tool for making node and wallet RPC calls.                                  |
| bin/novacoin-qt                                            | Novacoin node and wallet GUI                                                |
| bin/novacoin-tx                                            | Tool for creating and modifying transactions                                |
| bin/novacoin-util                                          | Miscellaneous utilities                                                     |
| bin/novacoin-wallet                                        | Novacoin wallet tool                                                        |
| bin/novacoind                                              | Novacoin node and wallet daemon                                             |
...
| *libexec/bench_novacoin*                                   | Benchmarking tool for measuring node performance                            |
| *libexec/novacoin-chainstate*                              | Tool to validate and connect blocks                                         |
| libexec/novacoin-gui                                       | IPC-enabled alternative to `novacoin-qt`                                    |
| libexec/novacoin-node                                      | IPC-enabled alternative to `novacoind`                                      |
| libexec/test_novacoin                                      | Unit test binary                                                            |
| *libexec/test_novacoin-qt*                                 | GUI-specific unit tests                                                     |
| share/man/man1/                                            | Man pages for command-line tools like `novacoin-cli`, `novacoind`, and others |
...
- README and novacoin.conf files are included in binary releases but not installed in source builds.
- On Windows, binaries have a `.exe` suffix (e.g., `novacoin-cli.exe`).
...
There have been multiple reports of database corruption and data loss when using this filesystem with Novacoin Core, see [Issue #31454](https://github.com/bitcoin/bitcoin/issues/31454) for more details.
